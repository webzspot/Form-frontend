...
// import { useFormContext } from "../dashboard/FormContext"; // Import the combined context
...
//       <WaveBackground position="top" color={isDarkMode ? "#4c1d95" : "#6c2bd9"} />
//       <WaveBackground position="bottom" color={isDarkMode ? "#4c1d95" : "#6c2bd9"} />
...
//          className={`${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-100'} rounded-3xl shadow-xl overflow-hidden border`}
//         >
//           {/* Header Banner */}
//       <div className={`h-32 relative ${isDarkMode ? 'bg-indigo-900' : 'bg-violet-600'}`}>
...
//               <div className={`flex items-center justify-between p-4 rounded-2xl ${isDarkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
...
//                     <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Adjust the appearance of the dashboard</p>
//                   </div>
//                 </div>
...
//                 <button 
//                   onClick={toggleTheme}
//                   className={`w-12 h-6 rounded-full relative transition-colors duration-300 ${isDarkMode ? 'bg-indigo-600' : 'bg-gray-300'}`}
//                 >
//                   <motion.div 
...
// // Updated InfoCard to handle Dark Mode
  ...
  return (
    ...
    <div className={`min-h-screen relative font-sans transition-colors duration-300 ${isDarkMode ? 'bg-[#0f172a]' : 'bg-[#F8FAFC]'}`}>
      ...
      <main className="max-w-5xl relative z-10 mx-auto mt-10 pb-12 px-4 sm:px-6">
        ...
        >
          {/* Header Banner */}
          <div className={`h-32 relative ${isDarkMode ? 'bg-indigo-950' : 'bg-violet-600'}`}>
             <div className={`absolute -bottom-12 left-8 p-1 rounded-2xl shadow-lg ${isDarkMode ? 'bg-slate-800' : 'bg-white'}`}>
                <div className={`w-24 h-24 rounded-xl flex items-center justify-center text-3xl font-bold ${isDarkMode ? 'bg-slate-700 text-indigo-400' : 'bg-indigo-50 text-indigo-600'}`}>
                  {user?.name?.charAt(0).toUpperCase()}
                </div>
