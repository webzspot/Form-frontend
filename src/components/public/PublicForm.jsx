 ...
 for (const field of form.formField) {
  ...
  try {
   ...
   setSuccessMessage("✅ Your response has been submitted successfully!");
    ...
    if (!form) {
    return (
      <div className="min-h-screen bg-purple-200 flex flex-col justify-center items-center">
         <div className="flex items-center gap-3 mb-6">
      <div className="w-10 h-10 bg-[#6C3BFF] rounded-xl flex items-center justify-center">
        <span className="text-white text-xl font-bold">⧉</span>
      </div>
      <h2 className="text-2xl font-bold text-[#0f172a]">FormCraft</h2>
    </div>

   
      <p className='text-purple-600 font-semibold text-lg'>Loading form...</p>  
      </div>
    )
  }
  ...
  <div className="min-h-screen bg-purple-200 flex  p-4">
    ...
    className="bg-[#f0ebf5] p-8 rounded-2xl flex flex-col justify-center items-center shadow-xl h-48 text-center max-w-md w-full">
      ...
      <button
        ...
        className="mt-4 bg-[#6C3BFF] hover:bg-[#5b2dfc] text-white py-2 px-4 rounded-lg font-semibold"
      >
  ...
  <form onSubmit={handleSubmit} className="bg-[#f0ebf5] p-8 rounded-2xl shadow-xl shadow-gray-600 w-full max-w-3xl">
    <h1 className="text-2xl font-extrabold text-violet-700 mb-1">{form.title}</h1>
    <p className="text-gray-800 text-lg mb-6">{form.description}</p>
      ...
      <div key={field.formFieldId} className="mb-4">
        ...
        {field.type === "TEXT" && (
          <input
            ...
            className="border border-gray-300 w-full px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6C3BFF]"
          />
        )}
        {field.type === "EMAIL" && (
          <input
    ...
    <button
      type="submit"
       disabled={submitting}
      className="mt-6 w-full bg-[#6C3BFF] text-white py-3 rounded-xl font-semibold hover:bg-[#5b2dfc]"
    >
