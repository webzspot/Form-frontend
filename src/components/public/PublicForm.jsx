...
const PublicForm = () => {
  ...
  useEffect(() => {
    const fetchPublicForm = async () => {
      ...
      formData.formField?.forEach(field => {
        if (field.defaultValue) {
          initialResponses[field.formFieldId] = field.defaultValue;
        }
      });
...
const theme = {
  ...
  const styles = {
    bg: theme.bgColor || "#f3f4f6",
    button: theme.buttonColor || "#2B4BAB",
    input: theme.inputBgColor || "#ffffff",
    font: theme.labelFont || "Inter",
    radius: theme.borderRadius || "16px",
    labelColor: theme.labelColor || "#374151"
                    ...
                    <div key={field.formFieldId} className="space-y-2">
                      ...
                      {(field.type === "TEXT" || field.type === "EMAIL" || field.type === "NUMBER") && (
                        ...
                        //  required={field.required}
                          ...
                          value={responses[field.formFieldId] || ""}
...
{/*        
  ...
  />
  <style>{`
      .date-input:not(:focus):not(.has-value):before {
...
{/* {field.type === "DATE" && (
  <input
    // Start as text to inherit all your standard text styles
    type={responses[field.formFieldId] ? "date" : "text"}
    placeholder={field.placeholder || "Select date"}
    readOnly={field.readOnly}
  //  required={field.required}
    
    // Switch to date on focus, switch back to text on blur if empty
    //onFocus={(e) => (e.target.type = "date")}
    onFocus={(e) => {
  if (!field.readOnly) e.target.type = "date";
}}
    onBlur={(e) => {
    ...
    style={{ 
      ...
      borderRadius: `calc(${styles.radius} / 2)`,
      color: styles.labelColor 
    }}
    
    
    className="w-full border border-gray-200 p-2 text-base outline-none focus:ring-2 focus:ring-opacity-50 transition-all shadow-sm"
  />
)} */}

{field.type === "DATE" && (
  <input
    // Logic: If there's a value (like a defaultValue), stay as "date". 
    // Otherwise, start as "text" to show the placeholder.
    type={responses[field.formFieldId] ? "date" : "text"}
    placeholder={field.placeholder || "Select date"}
    readOnly={field.readOnly}
    
    // Only allow switching to 'date' if not readOnly
    onFocus={(e) => {
      if (!field.readOnly) e.target.type = "date";
    }}
    onBlur={(e) => {
    ...
    // This connects the default value from your state to the input
    value={responses[field.formFieldId] || ""}
    onChange={(e) => handleInputChange(field.formFieldId, e.target.value)}
                        ...
                        <div className="grid grid-cols-1 gap-2">
                          {field.options?.map((opt, idx) => (
                            <label key={idx} className="flex items-center gap-3 p-2 border border-gray-100 rounded-xl cursor-pointer hover:bg-gray-50 transition-colors">
                              <input
                                type="radio"
                                disabled={field.readOnly} // Use disabled here
  checked={field.type === "RADIO" 
    ? responses[field.formFieldId] === opt 
    : (responses[field.formFieldId] || []).includes(opt)}
                                name={field.formFieldId}
                           //     required={field.required}
                                onChange={() => handleInputChange(field.formFieldId, opt)}
                                className="w-5 h-5"
