...
import axios from 'axios';
import { 
    FaPlus, FaSearch, FaTimes, FaFileAlt, FaRegEdit, FaTrash, FaCheck, FaUndo,FaTimesCircle
} from 'react-icons/fa';
import { motion, AnimatePresence } from "framer-motion";
import toast from "react-hot-toast";
import UserNavbar from '../user/UserNavbar';
...
const [errorMessage, setErrorMessage] = useState("");
    ...
    const GET_PLANS_URL = 'https://formbuilder-saas-backend.onrender.com/api/admin/payment/plans';
    const POST_PLAN_URL = 'https://formbuilder-saas-backend.onrender.com/api/admin/payment/plans';
    const ACTIVATE_PLAN_URL = 'https://formbuilder-saas-backend.onrender.com/api/admin/payment/plans/activate';
    const token = sessionStorage.getItem("token");
...
const confirmDelete = async () => {
  ...
  const res=await axios.delete(POST_PLAN_URL, {
            headers: { Authorization: `Bearer ${token}` },
            data: { planId: planToDelete.id } 
        });
        console.log(res)
...
const handleReactivate = async (planId) => {
    try {
        setIsSubmitting(true);
        await axios.patch(ACTIVATE_PLAN_URL, {planId:planId},
...
const filteredPlans = useMemo(() => {
    ...
    // 2. Sort a COPY of the results to avoid mutation warnings
    const sortedResult = [...result]; // Create a copy here

    if (sortOrder === "low-to-high") {
        sortedResult.sort((a, b) => a.amount - b.amount);
 ...
 <div className=" pr-2 py-2  rounded-md mb-8 flex flex-col md:flex-row gap-4 justify-between md:bg-[#FFFFFF] md:border md:border-[#E5E7EB] md:shadow-sm items-center">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
            ...
            onChange={(e) => setSortOrder(e.target.value)}
                className={`w-full md:w-auto px-2 py-2 cursor-pointer text-[10px] sm:text-sm font-bold rounded-md border outline-none  bg-white text-black`}
              >
                <option value="all">Default (Newest)</option>
        ...
        <motion.tr key={plan.id} className={`transition-colors border-b border-[#E9EAEB]  ${!plan.isActive ? 'bg-gray-50/50 opacity-80' : 'hover:bg-gray-50'}`}  initial={{ opacity: 0, x: -20 }}
            ...
            <td className="px-6 py-4 border-r border-[#E9EAEB]">
                <div className="flex  items-center gap-6">
                    <div className={`font-bold ${!plan.isActive ? 'text-gray-500' : 'text-[#181D27]'}`}>{plan.name}</div>
                    {/* The isActive check shows if the plan is archived/deleted */}
                    {!plan.isActive && (
                <span className="bg-amber-100 text-amber-700 text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider">
                    Archived
                </span>
            )}
                </div>
    ...
    <div className="flex justify-center gap-3">
      ...
      {plan.isActive === false ? (
            <button 
                ...
                className="text-gray-400 hover:text-amber-700 transition-all p-2 hover:bg-amber-50 rounded-full"
                title="Reactivate Plan"
            >
                <FaUndo size={15} /> {/* Or FaUndo */}
            </button>
        ) : (
            <button 
                onClick={() => openDeleteModal(plan)}
            ...
            Next
                    ...
                    <div className="md:hidden space-y-4">
                        ...
                        currentData.map((plan) => (
                            <motion.div 
                                initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                               className={`p-4 rounded-md border border-[#E9EAEB] shadow-sm transition-all ${
                    plan.isActive === false 
                    ? "bg-gray-50/80 opacity-70 grayscale-[0.3]" 
                    : "bg-white opacity-100"
                }`}
                                key={plan.id}
                            >
                                <div className="flex justify-between items-start mb-3">
                                    <span className={`px-2 rounded text-[10px] font-bold uppercase ${
                        plan.isActive === false 
                        ? "bg-gray-200 text-gray-500" 
                        : "bg-[#2B4BAB]/10 text-[#2B4BAB]"
                    }`}>{plan.planType}</span>
                                    <h3 className={`font-normal text-base ${plan.isActive === false ? "text-gray-500" : "text-[#181D27]"}`}>{plan.currency} {plan.amount}</h3>
                                </div>
                                <h5 className="font-bold text-[#181D27] mb-1">
  
    {plan.isActive === false && (
        <span className="ml-2 bg-amber-100 text-amber-700 text-[8px] px-1 py-0.5 font-bold">
            Archived
        </span>
    )}
</h5>
                                <div className='flex justify-between items-center'>
                                    <div className='flex flex-col gap-2'>
                                <h4 className={`font-bold ${plan.isActive === false ? "text-gray-500" : "text-[#181D27]"}`}>{plan.name}</h4>
                                <p className="text-xs text-[#6A7181] mb-2">{plan.description}</p>
                                 </div>
                                  <div className="flex justify-center gap-3">
