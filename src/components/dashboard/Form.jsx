...
// // // const getUser = useCallback(async () => {
// // //   setLoading(true); // Start skeletons
// // //   try {
...
// // //     // 2. IMMEDIATE CHECK: Is the user suspended?
// // //     if (res.data?.data?.AccountStatus === "SUSPENDED") {
// // //       setApiError(403);
// // //       setErrorMessage("Your account is suspended. Please contact admin.");
// // //       // We stop here and don't fetch anything else
// // //       return; 
// // //     }

// // //     // 3. If NOT suspended, save user data
// // //     setUserData(res.data?.data);
// // //     setApiError(null);

// // //     // 4. NOW fetch the other two APIs and wait for them
// // //     // Note: Remove 'setLoading' calls from inside these two functions!
// // //     await Promise.all([
// // //       getMasterFields(),
// // //       fetchForms(),
// // //     ]);
...
// // //   } catch (error) {
// // //     setApiError(error.response?.status || 500);
// // //     setErrorMessage(error.response?.data?.message || "Error loading profile.");
// // //   } finally {
// // //     // 5. FINALLY turn off skeletons when all 3 APIs are done
// // //     setLoading(false);
// // //   }
// // // }, [token, navi]);
// // const getUser = useCallback(async () => {
...
// //     setUserData(res.data?.data);
// // //console.log(res)
// //     // If one of these fails, it jumps straight to the catch below
// //     await Promise.all([
// //       getMasterFields(),
// //       fetchForms(),
// //     ]);
...
// //   } catch (error) {
// //     // This is where your 404 or 500 error is saved
// //     setApiError(error.response?.status || 500);
// //     setErrorMessage(error.response?.data?.message || "Something went wrong.");
// //   } finally {
// //     // This turns off the skeletons so the Error Screen can show
// //     setLoading(false);
// //   }
// // }, [token, navi]);
...
// // //       <div className="opacity-40 grayscale w-full pointer-events-none select-none blur-[1px] transition-all">
// // //         {children}
// // //       </div>
...
// // // const FeatureGuard = ({ userData, children }) => {
  
// // //   const hasAccess = userData?.limits?.themeAccess === true;
...
// // //         <div className="bg-white/90 p-2 rounded-full shadow-md border border-gray-100">
// // //           <Lock className="w-4 h-4 text-[#2B4BAB]" />
// // //         </div>
// // //       </div>
// // //     </div>
// // //   );
...
// //     // Prevent the click from reaching the underlying (blurred) content
// //     e.preventDefault();
// //     e.stopPropagation();
    
// //     // Trigger the notification
// //     toast.error("Theme is not available. Try to update your plan!");
// //   };
...
// //     >
// //       {/* Removed 'pointer-events-none' so the parent can catch the click.
// //           Added 'select-none' to prevent text highlighting.
// //       */}
// //       <div className="opacity-40 grayscale select-none blur-[1px] transition-all">
// //         {children}
// //       </div>

// //       {/* Overlay Icon */}
// //       <div className="absolute inset-0 z-20 flex items-center justify-center">
// //         <div className="bg-white/90 p-2 rounded-full shadow-md border border-gray-100 group-hover:scale-110 transition-transform">
// //           <Lock className="w-4 h-4 text-[#2B4BAB]" />
// //         </div>
// //       </div>
// //     </div>
...
// // // //   const hasAccess = userData?.themeAccess === true;
...
// //              className={`w-full rounded mt-6 flex flex-col lg:flex-row overflow-hidden
// //   ${theme.card}
// // `}  >
...
// //   className="space-y-2.5 overflow-y-auto max-h-[40vh] lg:max-h-[50vh] no-scrollbar"
// //   style={{ 
// //     width: '100%',
...
// //       <div 
// //         key={`skeleton-${i}`}
// //         className="flex items-center gap-3 py-3 px-4 bg-gray-100 rounded-sm animate-pulse"
// //       >
// //         <div className="w-5 h-5 bg-gray-200 rounded-full" /> {/* Icon placeholder */}
// //         <div className="flex-1">
// //           <div className="h-4 bg-gray-200 rounded w-3/4" /> {/* Text placeholder */}
// //         </div>
// //         <div className="w-4 h-4 bg-gray-200 rounded" /> {/* Grip placeholder */}
// //       </div>
// //     ))
// //   ):masterFields.length === 0 ? (
// //     // --- EMPTY STATE ---
// //     <div className="flex flex-col items-center pt-20 justify-center py-10 border-2 border-dashed border-gray-100 rounded-sm bg-gray-50/50">
    
// //       <p className="text-gray-500 text-base font-medium">No masterfields found</p>
// //       <p className="text-gray-400 text-[10px]">Add your first field below</p>
// //     </div>
// //   ) :(
...
// // <div className={`transition-colors duration-300 ${isSelected ? "text-gray-400" : "text-gray-400"}`}>
// //         {fieldConfig.icon}
// //       </div>
...
// //       whileTap={{ scale: 0.99 }}
// //       className={` py-2.5  w-full  rounded-sm font-semibold flex items-center justify-center gap-3 transition-all duration-300 border-2 ${
// //         isAddingMaster
...
// //        <FeatureGuard userData={currentPlan}>
// //           <Design
// //             editingFormId={editingFormId}
// //             token={token}
// //             formTheme={formTheme}
...
// //           />
// //        </FeatureGuard>
// //        {/* <FeatureGuard userData={userData}>
// //   <Design
// //     editingFormId={editingFormId}
// //     token={token}
// //     formTheme={formTheme}
...
// //               transition={{ type: "spring", damping: 25, stiffness: 300 }}
// //               className="relative w-[350px] sm:w-[459px] max-w-2xl max-h-[90vh] overflow-hidden rounded-xl shadow-2xl flex flex-col border border-gray-100"
// //               style={{
...
// //                       toast.error("Form is draft. Make it live to copy link.")
// //                       return
// //                     }
...
// //     onClick={() => {
// //       if (!viewData.isPublic) {
// //         toast.error("Form is draft. Make it live first.");
// //         return;
// //       }
// //       setShowIframeMenu(!showIframeMenu);
// //     }}
// //     style={{
// //       background: viewData.isPublic ? "#2B4BAB" : "#E5E7EB",
// //       color: viewData.isPublic ? "white" : "#9CA3AF",
// //     }}
// //     className={`w-full text-xs px-2 py-1 md:px-3 md:py-2 rounded-sm font-bold flex items-center justify-center gap-1 ${
// //       viewData.isPublic ? "shadow-lg" : "cursor-not-allowed"
// //     }`}
// //   >
// //     iframe <ChevronDown size={14} />
// //   </motion.button>
...
// //           <h3 className="text-lg font-bold text-gray-800">Select Embed Type</h3>
// //           <button onClick={() => setShowIframeMenu(false)} className="text-gray-400 hover:text-gray-600">
// //             <X size={20} />
// //           </button>
// //         </div>
        
// //         <p className="text-sm text-gray-500 mb-6">Choose the format that matches your project type.</p>
...
// //               const htmlCode = `<iframe src="${formLink}" title="${viewData.title}" width="100%" height="700" style="border:none; border-radius:12px;"></iframe>`;
// //               await navigator.clipboard.writeText(htmlCode);
// //               toast.success("HTML Code Copied!");
// //               setShowIframeMenu(false);
// //             }}
// //             className="flex flex-col items-start p-4 border-2 border-gray-100 rounded-lg hover:border-[#2B4BAB] hover:bg-blue-50 transition-all text-left group"
// //           >
// //             <span className="font-bold text-[#2B4BAB]">HTML Code</span>
// //             <span className="text-xs text-gray-500">Best for WordPress, Webflow, or static sites.</span>
// //           </button>

// //           {/* React Option */}
// //           <button
// //            // React Option inside the Modal
// // onClick={async () => {
// //   const baseUrl = import.meta.env.VITE_URL?.replace(/\/$/, "") || window.location.origin;
// //   const formLink = `${baseUrl}/public/form/${viewData.slug}`;
  
// //   // This is JUST the iframe tag, but using React's style object syntax
// //   const reactTagOnly = `<iframe \n  src="${formLink}" \n  title="${viewData.title}" \n  width="100%" \n  height="700" \n  style={{ border: 'none', borderRadius: '12px' }} \n/>`;
  
// //   await navigator.clipboard.writeText(reactTagOnly);
// //   toast.success("React Iframe Tag copied!");
// //   setShowIframeMenu(false);
// // }}
// //             className="flex flex-col items-start p-4 border-2 border-gray-100 rounded-lg hover:border-[#2B4BAB] hover:bg-blue-50 transition-all text-left group"
// //           >
// //             <span className="font-bold text-[#2B4BAB]">React</span>
// //             <span className="text-xs text-gray-500">Best for React.js, Next.js, or Vite projects.</span>
// //           </button>
// //         </div>
// //       </motion.div>
// //     </div>
...
// //         >
// //           <div className="flex justify-between items-center mb-4">
// //             <h3 className="text-lg font-bold text-gray-800">Select Script Type</h3>
// //             <button onClick={() => setShowScriptMenu(false)} className="text-gray-400 hover:text-gray-600">
// //               <X size={20} />
// //             </button>
// //           </div>
...
// //                 toast.success("HTML Script Copied!");
// //                 setShowScriptMenu(false);
// //               }}
// //               className="flex flex-col items-start p-4 border-2 border-gray-100 rounded-lg hover:border-[#2B4BAB] hover:bg-blue-50 transition-all text-left group"
// //             >
// //               <span className="font-bold text-[#2B4BAB]">HTML Code
// //               </span>
// //               <span className="text-xs text-gray-500">Best for WordPress, Webflow, or static sites.</span>
// //             </button>
...
// //   // This copies the WHOLE file structure so the user just pastes it in
// //   const reactSnippet = `import React, { useEffect, useRef } from 'react';
...
// //     script.src = "${baseUrl}/publicFormEmbed.js";
// //     script.async = true;
    
// //     // Data attributes for theme and ID
// //     script.dataset.formId = "${viewData.slug}";
// //     script.dataset.primaryColor = "${viewData.theme?.buttonColor || "#7C3AED"}";
// //     script.dataset.bgColor = "${viewData.theme?.bgColor || "#ffffff"}";
// //     script.dataset.font = "${viewData.theme?.labelFont || "Inter"}";
// //     script.dataset.inputBgColor = "${viewData.theme?.inputBgColor || "#ffffff"}";
// //     script.dataset.labelColor = "${viewData.theme?.labelColor || "#374151"}";
// //     script.dataset.borderRadius = "${viewData.theme?.borderRadius || "16px"}";
// //     script.dataset.width = "100%";

// //     // Attach to our specific div instead of document.body
...
// //   toast.success("Complete React Component Copied!");
// //   setShowScriptMenu(false);
// // }}
// //               className="flex flex-col items-start p-4 border-2 border-gray-100 rounded-lg hover:border-[#2B4BAB] hover:bg-blue-50 transition-all text-left group"
// //             >
// //               <span className="font-bold text-[#2B4BAB]">React</span>
// //               <span className="text-xs text-gray-500">Best for React.js or Vite projects.</span>
// //             </button>
// //           </div>
// //         </motion.div>
// //       </div>
...
// //    // REPLACE your current reactSnippet definition with this:
// // const reactSnippet = `import React, { useEffect, useRef } from 'react';
...
// //     const scriptId = "form-script-${viewData.slug}";
...
// //       if (existingScript && containerRef.current && containerRef.current.contains(existingScript)) {
...
//   const [formTitle, setFormTitle] = useState("Untitled Form")
//   const [isAddingMaster, setIsAddingMaster] = useState(false)
//   // UPDATED: added placeHolder, readOnly, required to newField initial state
//   const [newField, setNewField] = useState({ label: "", type: "TEXT", options: [""], placeHolder: "", readOnly: false, required: false })
//   const [introloading, setintroLoading] = useState(true);
//   const [isSavingMasterField, setIsSavingMasterField] = useState(false);
//   const [apiError, setApiError] = useState(null);
// const [errorMessage, setErrorMessage] = useState("");
...
//       setNewField({ label: "", type: "TEXT", options: [""], placeHolder: "", readOnly: false, required: false })
//       setIsAddingMaster(false)
//     } catch (err) {
//       toast.error("Failed to add master field")
//     }finally {
...
//   // 2. Toggle field selection — carries placeHolder & readOnly from master field
//   const toggleField = (toggledField) => {
//     setSelectedFields((prev) => {
//       const exists = prev.find((f) => f.masterFieldId === toggledField.masterFieldId)
//       if (exists) {
...
//       return [...prev, { 
//         ...toggledField, 
//         required: toggledField.required || false,
//         placeHolder: toggledField.placeHolder || "",
//         readOnly: toggledField.readOnly || false,
//       }]
//     })
//   }
...
//           whileHover={{ x: 4 }}
//           whileTap={{ scale: 0.98 }}
//           className={`group flex flex-col gap-1 py-3 px-4 transition-all duration-300 rounded-sm cursor-pointer ${
//             isSelected
//               ? "border-[2px] border-[#2B4BAB]" 
//               : "bg-white hover:border-gray-200"
//           }`}
...
//           {/* Top row: icon + label + grip */}
//           <div className="flex items-center gap-3">
//             <div className={`transition-colors duration-300 ${isSelected ? "text-gray-400" : "text-gray-400"}`}>
//               {fieldConfig.icon}
//             </div>
//             <div className="flex-1 min-w-0">
//               <span className={`font-semibold block truncate transition-colors duration-300 ${isSelected ? "text-gray-700" : "text-gray-700"}`}>
//                 {field.label}
//               </span>
//             </div>
//             <GripVertical className={`w-4 h-4 transition-all duration-300 ${
...
//           {/* UPDATED: Show badges for placeholder, required, readOnly under the label */}
//           <div className="flex flex-wrap gap-1 pl-7">
//             {field.placeHolder && (
//               <span className="text-[10px] bg-blue-50 text-blue-600 border border-blue-100 px-1.5 py-0.5 rounded font-medium truncate max-w-[120px]" title={field.placeHolder}>
//                  {field.placeHolder}
//               </span>
//             )}
//             {field.required && (
...
//             {/* UPDATED: Required & ReadOnly toggles */}
//             <div className="flex gap-3">
//               {/* Required Toggle */}
//               <motion.div
//                 whileHover={{ scale: 1.02 }}
...
//                 className={`flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-sm border-2 cursor-pointer transition-all duration-300 ${
//                   newField.required
...
//                   newField.required ? "bg-red-500 border-red-500" : "border-gray-300"
//                 }`}>
//                   {newField.required && <Check className="w-2.5 h-2.5 text-white" />}
//                 </div>
...
//                 whileHover={{ scale: 1.02 }}
//                 whileTap={{ scale: 0.98 }}
//                 onClick={() => setNewField({ ...newField, readOnly: !newField.readOnly })}
//                 className={`flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-sm border-2 cursor-pointer transition-all duration-300 ${
//                   newField.readOnly
...
//               >
//                 <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all ${
//                   newField.readOnly ? "bg-amber-500 border-amber-500" : "border-gray-300"
//                 }`}>
//                   {newField.readOnly && <Lock className="w-2.5 h-2.5 text-white" />}
//                 </div>
//                 <span className="text-[11px] font-bold uppercase tracking-wider">Read-only</span>
//               </motion.div>
//             </div>
...
//                             {/* UPDATED: Placeholder field in selected field card */}
//                             <div className="mb-4">
//                               <label className="text-xs uppercase font-bold ml-1 mb-2 block text-gray-400">
//                                 Placeholder
//                               </label>
//                               <input
//                                 type="text"
...
//                                   color: "#2B4BAB",
//                                 }}
//                                 className="w-full border-2 sm:px-3 sm:py-2 px-2 py-1 rounded-lg text-sm outline-none transition-all border-gray-200"
//                               />
//                             </div>
...
//                               {/* UPDATED: ReadOnly Toggle in selected field card */}
//                               <motion.div
//                                 whileHover={{ scale: 1.02 }}
//                                 whileTap={{ scale: 0.98 }}
//                                 className={`flex items-center gap-3 px-4 py-2 rounded-full cursor-pointer transition-all duration-300 ${
...
//                                     : "bg-gray-50 border-2 border-gray-100 hover:border-gray-200"
//                                 }`}
//                                 onClick={() => updateFieldProperty(index, "readOnly", !field.readOnly)}
//                               >
//                                 <div
//                                   className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all ${
//                                     field.readOnly ? "bg-amber-500 border-amber-500" : "border-gray-300"
//                                   }`}
//                                 >
//                                   {field.readOnly && <Lock className="w-3 h-3 text-white" />}
//                                 </div>
//                                 <span
//                                   className={`text-[10px] sm:text-sm font-bold uppercase ${field.readOnly ? "text-amber-600" : "text-gray-500"}`}
//                                 >
//                                   Read-only
//                                 </span>
//                               </motion.div>
...
//                         <span className="flex items-center gap-2">
//                           <span className="w-2 h-2 rounded-full bg-[#2B4BAB] animate-pulse" />
//                           {field.placeHolder || `Preview for ${field.type}`}
//                         </span>
//                         <ChevronRight className="w-4 h-4" />
//                       </div>
//                     </motion.div>
...
//   src="${baseUrl}/publicFormEmbed.js"
//   data-form-id="${viewData.slug}"
//   data-primary-color="${viewData.theme?.buttonColor || "#2B4BAB"}"
//   data-bg-color="${viewData.theme?.bgColor || "#ffffff"}"
//   data-font="${viewData.theme?.labelFont || "Inter"}"
//   data-input-bg-color="${viewData.theme?.inputBgColor || "#ffffff"}"
//   data-label-color="${viewData.theme?.labelColor || "#374151"}"
...
//     script.dataset.formId = "${viewData.slug}";
//     script.dataset.primaryColor = "${viewData.theme?.buttonColor || "#2B4BAB"}";
//     script.dataset.bgColor = "${viewData.theme?.bgColor || "#ffffff"}";
//     script.dataset.font = "${viewData.theme?.labelFont || "Inter"}";
//     script.dataset.inputBgColor = "${viewData.theme?.inputBgColor || "#ffffff"}";
//     script.dataset.labelColor = "${viewData.theme?.labelColor || "#374151"}";
...
const getUser = useCallback(async () => {
  ...
  setSelectedFields((prevFields) => {
    ...
    // 1. Update the property the user clicked (e.g., readOnly)
    updatedFields[index][property] = value;

    // 2. SMART LOGIC: If ReadOnly becomes false, clear the Default Value automatically
    if (property === "readOnly" && value === false) {
  ...
  const createForm = async () => {
    if (selectedFields.length === 0) return toast.error("Please select at least one field")
      const missingDefault = selectedFields.find(f => f.readOnly && !f.defaultValue);
  if (missingDefault) {
    return toast.error(`Please provide a Default Value for "${missingDefault.label}" because it is Read Only.`);
  }
    setLoading(true)
    try {
      const uniqueTitle = formTitle === "Untitled Form" ? `Untitled Form ${Date.now()}` : formTitle
      const payload = {
        ...
        fields: selectedFields.map((field, index) => ({
          ...
          placeHolder:field.placeHolder,
          readOnly:field.readOnly,
         defaultValue: field.defaultValue ? field.defaultValue.toString() : "",
          order: index,
          masterFieldId: field.masterFieldId || null,
          options: field.options || null,
        })),
      ...
      if (err.response?.status === 403 || err.response?.data?.message?.includes("limit")) {
      toast.error("You've reached the limit of creating forms. Upgrade the plan for unlimited forms!");
    } else {
      toast.error("Something went wrong");
    }
    } finally {
      ...
      {isAddingMaster && (
        ...
        >
          <div className="p-5 bg-gray-50 border border-gray-100 rounded-sm space-y-4 shadow-inner">
            ...
            {/* UPDATED: Placeholder field */}
           {newField.type !== "CHECKBOX" && newField.type !== "RADIO" && ( <div className="space-y-2">
              <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                Placeholder
              </label>
              <input
...
`}
                            ...
                            {/* UPDATED: Placeholder field in selected field card */}
                        {field.type !== "CHECKBOX" && field.type !== "RADIO" && ( <div className="mb-4">
                              <label className="text-xs uppercase font-bold ml-1 mb-2 block text-gray-400">
                                Placeholder
                              </label>
                              <input
  ...
  {/* Conditional Default Value Input - Only shows on the right side when Read Only is true */}
{field.readOnly && (
  <div className="mt-3 p-3 bg-blue-50/50 border border-blue-100 rounded-sm space-y-2">
    <label className="block text-[10px] font-bold text-[#2B4BAB] uppercase tracking-widest">
      Default Value <span className="text-red-500">*</span>
    </label>
    
    {field.type === "DROPDOWN" || field.type === "RADIO" ? (
      <select
        className="w-full border border-gray-200 rounded-sm text-sm px-3 py-2 outline-none bg-white"
        value={field.defaultValue || ""}
        onChange={(e) => updateFieldProperty(index, "defaultValue", e.target.value)}
      >
      ...
      <div className="flex flex-wrap gap-2 bg-white p-2 border border-gray-100 rounded-sm">
        {field.options?.map((opt, i) => (
          <label key={i} className="flex items-center gap-1 text-xs text-gray-600 cursor-pointer">
            <input
              type="checkbox"
              // Check if option exists in the string (comma separated)
              checked={field.defaultValue?.split(',').includes(opt)}
              onChange={(e) => {
                const currentVals = field.defaultValue ? field.defaultValue.split(',').filter(v => v !== "") : [];
                let newVals;
                if (e.target.checked) {
                ...
                // Save back as a string like "Option1,Option2"
                updateFieldProperty(index, "defaultValue", newVals.join(','));
              }}
            />
            {opt}
          </label>
      ...
      <input
        ...
        className="w-full border border-gray-200 rounded-sm text-sm px-3 py-2 outline-none bg-white"
        value={field.defaultValue || ""}
        // Smart Instructions/Placeholders based on type
        placeholder={
          field.type === "EMAIL" ? "eg: user@example.com" :
          field.type === "NUMBER" ? "eg: 12345" :
          field.type === "TEXTAREA" ? "Enter default long text..." :
          "Enter value..."
        }
        onChange={(e) => updateFieldProperty(index, "defaultValue", e.target.value)}
      />
    ...
    <p className="text-[9px] text-gray-400 italic">
      This value will be pre-filled and locked for the user.
    </p>
  </div>
)}
                              <span className="sm:text-xs text-[10px] text-gray-400 italic bg-gray-50 px-3 py-1.5 rounded-full">
