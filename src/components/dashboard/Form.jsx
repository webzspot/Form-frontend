...
const formview = async (formId) => {
  setViewLoadingId(formId)  // store the specific id
  try {
    const response = await axios.get(`https://formbuilder-saas-backend.onrender.com/api/dashboard/form/details/${formId}`,
      { headers: { Authorization: `Bearer ${token}` } }
    )
    ...
    setViewLoadingId(null)  // clear it
  }
}
...
const displayedForms = filteredForms.slice(
...
<motion.button
  onClick={() => formview(form.formId)}
  disabled={viewLoadingId === form.formId}
  className={`flex-1 py-2.5 rounded-lg font-medium text-sm text-white ${theme.buttonPrimary} transition-all`}
>
  {viewLoadingId === form.formId 
...
{!loading && totalPages > 1 && (
  ...
  >
    <button
      ...
      className="px-4 py-2 rounded-md bg-white border border-gray-200 text-gray-600 font-semibold text-sm hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
    >
      ← Prev
    </button>
