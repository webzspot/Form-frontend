...
//                 onClick={saveThemeOnly} 
//                 disabled={loading} 
//                 className={`w-full py-4 rounded-sm text-xs font-bold  shadow-md ] ${
//                   editingFormId ? "bg-[#2B4BAB] text-white hover:bg-black" : "bg-gray-300 text-white cursor-not-allowed"
//                 }`}
//               >
//                 {loading ? "Syncing..." : "Finalize Design"}
...
const Design = ({ editingFormId, token, formTheme, setFormTheme, isDarkMode }) => {
  ...
  return (
    <div className="w-full mb-4">
      <button
        onClick={() => setIsDesigning(!isDesigning)}
        className="bg-[#2B4BAB] text-white px-4 py-3 rounded-md font-semibold flex items-center gap-3 w-full justify-center transition-all shadow-lg"
      >
      ...
      <AnimatePresence>
        {isDesigning && (
          <motion.div
            ...
            animate={{ height: "auto", opacity: 1 }}
            exit={{ height: 0, opacity: 0 }}
            className={`mt-4 ${theme.card} rounded-md shadow-2xl overflow-hidden flex flex-col`}
          >
            {/* --- TAB NAVIGATION --- */}
            <div className={`flex ${theme.previewBox} p-1`}>
              {[
                ...
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`flex-1 flex flex-col items-center sm:py-3 py-1 gap-1 transition-all rounded-md ${
                    activeTab === tab.id 
                    ? `${theme.buttonPrimary}` 
                    : "text-gray-400 hover:text-[#2B4BAB]"
                  }`}
                  ...
                  >
                    {palettes.map(p => (
                      <button 
                        key={p.name} 
                        onClick={() => applyPalette(p)} 
                        className={`group sm:p-3 p-2 ${theme.input} rounded-md flex flex-col gap-3 transition-all text-left`}
                      >
                        <div className="flex gap-1">
                          <div className="sm:w-6 sm:h-6 w-4 h-4 rounded-full shadow-sm border border-gray-200" style={{ backgroundColor: p.bg }}></div>
                          <div className="sm:w-6 sm:h-6 w-4 h-4 rounded-full shadow-sm" style={{ backgroundColor: p.btn }}></div>
                ...
                {activeTab === 'background' && (
                  ...
                  >
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-4 items-center">
                        <input type="color" value={formTheme.bgColor} onChange={(e) => handleLiveUpdate('bgColor', e.target.value)} className="w-20 h-20 cursor-pointer rounded-2xl border-none" />
                        <div className={`flex-1 p-3 text-[11px] sm:text-sm rounded-md ${theme.input}`}>
                          This color applies to the entire page background of your public form.
                        </div>
                      </div>
                    </div>
                ...
                {activeTab === 'typography' && (
                  ...
                  >
                        ...
                        <div className="grid grid-cols-1 gap-2 mt-4">
                            {["Inter", "Poppins", "Roboto", "Montserrat", "Playfair Display", "Space Grotesk"].map((font) => (
                                <button
                                    ...
                                    onClick={() => handleLiveUpdate('labelFont', font)}
                                    style={{ fontFamily: font }}
                                    className={`w-full text-[12px] sm:text-sm px-4 sm:py-2 py-0.5 text-left rounded-md transition-all ${formTheme.labelFont === font ? "bg-[#2B4BAB] text-white shadow-sm" : `${theme.texttwo} hover:bg-gray-100`}`}
                                >
                                    {font}
                                </button>
                            ))}
                    ...
                    <div className="space-y-3">
                        <label className={`text-[12px] sm:text-sm uppercase font-bold ${theme.label}`}>Text Color</label>
                        <input type="color" value={formTheme.labelColor || "#374151"} onChange={(e) => handleLiveUpdate('labelColor', e.target.value)} 
                        className="w-full mt-2 sm:h-10 h-8 cursor-pointer rounded-md border-none" />
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            <div className={`p-5 ${theme.input} border-t border-gray-100`}>
              <button 
                onClick={saveThemeOnly} 
                disabled={loading} 
                className={`w-full sm:py-4 py-2 rounded-md text-sm font-bold transition-all shadow-lg active:scale-95 ${editingFormId ? `${theme.buttonPrimary}` : `${theme.buttonsecondary}`}`}
              >
                {loading ? "Saving Styles..." : "Apply & Save Design"}
              </button>
            </div>
